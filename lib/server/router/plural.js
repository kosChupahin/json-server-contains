...
module.exports = (db, name, opts) => {
  ...
  function handleContainsQueries(chain, req, key) {
    const params = [].concat(req.query[key]);
    const path = key.replace(/(_contains)$/, '');
  ...
  function handleDefaultQueries(chain, req, key) {
  ...
  function list(req, res, next) {
    ...
    Object.keys(req.query).forEach(key => {
      ...
      if (key !== 'callback' && key !== '_') {
        ...
        if (isContains) {
          chain = handleContainsQueries(chain, req, key);
        } else {
          chain = handleDefaultQueries(chain, req, key);
        }
      }
    }); // Sort
